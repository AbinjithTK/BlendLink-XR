...
BL_COLLECTION_NAME = "BlendLink XR"
...
def link_to_configured_collection(ob):
    ...
    collection = getattr(scene, "blendlinkxr_draw_collection", None) or scene.collection
...
def get_blendlink_collection():
    if BL_COLLECTION_NAME not in bpy.data.collections:
        c = bpy.data.collections.new(BL_COLLECTION_NAME)
        bpy.context.scene.collection.children.link(c)

    return bpy.data.collections[BL_COLLECTION_NAME]


def gc_blendlink_collection():
    if BL_COLLECTION_NAME in bpy.data.collections:
        coll = bpy.data.collections[BL_COLLECTION_NAME]
...
def recenter_mouse_cursor():
    "Moves the mouse into the center of region3d"

    from blendlinkxr.utils import desktop_viewport

    region = desktop_viewport.get_region()

    region_center_x = region.width // 2
    region_center_y = region.height // 2

    window_center_x = region_center_x + region.x
    window_center_y = region_center_y + region.y

    bpy.context.window.cursor_warp(window_center_x, window_center_y)
