...
from bl_xr.events.make_events.click_drag import is_within_threshold
...
def _duplicate(obj, linked=False):
    obj_copy = obj.copy()
    if not linked:
...
def on_clone_start(self, event_name, event):
    global clone_press_start_time, clone_state, object_being_transformed

    if object_being_held and object_being_transformed:
        ...
        object_being_transformed = False  # clear this flag, since we've accounted for it
...
def on_clone_press(self, event_name, event):
    ...
    from blendlinkxr import gizmos

    if "joystick_for_keyframe" in gizmos.active_gizmos:  # HACK
        if log.isEnabledFor(logging.DEBUG):
            log.debug("Bailing out of clone since joystick_for_keyframe gizmo is enabled!")
        return
    ...
    if clone_state == "CLONED" or (
        ...
        if log.isEnabledFor(logging.DEBUG):
            log.debug(
                f'Bailing out of clone. {clone_state=}, {object_being_held=}, {time.time()} < {clone_press_start_time} + {settings["clone.long_press_threshold_time"]}'
            )
        return
    ...
    if ob is None or ob.mode != "OBJECT":
        if log.isEnabledFor(logging.DEBUG):
            log.debug(f"Bailing out of clone. {ob=} {ob.mode if ob else ''}!")
        return
    ...
    if len(selected_objects) == 0:
        if log.isEnabledFor(logging.DEBUG):
            log.debug("Bailing out of clone since nothing is selected!")
        return
    ...
    for ob in selected_objects:
        ob_copy = _duplicate(ob, linked=settings["clone.linked_duplicates"])
        log.info(f"CLONED: {ob_copy} FROM {ob}")

        # move slightly
        if object_being_held is None:
            ob_copy.location += 0.1 * VEC_ONE

        # swap selection
        ob.select_set(False)
        ob_copy.select_set(True)

        bpy.context.view_layer.objects.active = ob_copy
        cloned_objects.append(ob_copy)

    if object_being_held is not None and len(cloned_objects) > 0:
        object_being_held = cloned_objects[0].name

    root.dispatch_event("fb.clone", cloned_objects)
...
def on_object_drag(self, event_name, event):
    ...
    if not is_within_threshold(event.pose_delta):
...
def clear_held_object():
    ...
    if object_being_held and object_being_transformed and clone_state == "CLONED":
        bpy.ops.ed.undo_push(message="clone")
...
def enable_tool():
    ...
    Object.add_event_listener("drag_start", on_object_drag)
    Object.add_event_listener("drag", on_object_drag)
...
def disable_tool():
    ...
    Object.remove_event_listener("drag_start", on_object_drag)
    Object.remove_event_listener("drag", on_object_drag)
