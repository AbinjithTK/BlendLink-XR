...
from bl_xr.utils import vec_signed_angle, get_bmesh_elements, get_bmesh, quat_diff, is_equal
...
def reset_transform_state():
    """Reset transform state to default values."""
    T.update({
        ...
        "lock_bone_position": False,
        "is_transforming_from_grab": False,
    })

reset_transform_state()

def configure_transform_targets(self, event):
    ...
    ob = list(event_targets)[0]

    if (
        self.type == "MESH"
        and self.mode == "EDIT"
        and len(cursor_targets) > 0
        and not isinstance(list(cursor_targets)[0], tuple)
    ):
        cursor_targets = [(e.index, type(e)) for e in cursor_targets]  # don't store bmesh object references

    # get the elements that are already selected and within the cursor
    selected_cursor_targets = get_selected_elements(cursor_targets, active_ob=ob)

    # get the overall collection of selected elements
    # factors in the object mode so it only looks at mesh elements if in edit mesh
    T["preselected_targets"] = get_selected_elements(active_ob=ob)

    T["transform_elements"] = set(cursor_targets)
    if len(selected_cursor_targets) > 0:
        # we include preselected elements, because all the selected items are considered
        # as one "preselected group" to transform, even if some are outside the cursor
...
def is_transform_allowed(self, event):
...
def on_transform_edit_mesh(ob, event_name, event):
    ...
    if tool_settings.use_proportional_edit:
        ...
        bm = get_bmesh()
        for i, _ in transform_state["transform_elements"]:
            v = bm.verts[i]
...
def on_transform_edit_curve(ob, event_name, event):
    scale = event.pose_delta.scale_factor
    if isinstance(scale, Vector):
        scale = _get_scale_component(scale)
...
def _get_scale_component(v: Vector):
    if not is_equal(v.x, 1):
        return v.x
    if not is_equal(v.y, 1):
        return v.y
    return v.z
...
def on_transform_edit_armature(ob, event_name, event):
    ...
    for bone, el_type, bone_head, bone_tail, bone_rot, bone_length in sub_targets:
        ...
        is_corner = el_type != "BOTH" and len(sub_targets) == 1
        transform_bone(
            bone,
            el_type,
            transform_m,
...
def on_transform_pose_armature(ob, event_name, event):
    ...
    for b, _ in transform_state["transform_elements"]:
        if len(b.constraints) > 0 or b.ik_stretch > 0:
...
def transform_bones_with_constraints(bones, transform_m, transform_m_inv, world_rot, event, delta_to_apply):
    ...
    def transform_movable_bones(movable_bones, transform_m, transform_m_inv, world_rot, event, delta_to_apply):
        ...
        pivot_position = event.pivot_position

        if settings["gizmo.transform_handles.type"] == "rotate":
            T["transform_gizmo"].update()  # to reflect the new selection

            base_pos = T["transform_gizmo"].position_world
            base_rot = T["transform_gizmo"].rotation_world

            delta_to_apply = Pose(rotation=pivoted_rotate(base_pos, base_rot, pivot_position, delta_to_apply))
            pivot_position = Vector(base_pos)

        # print(delta_to_apply.position)

        with transform_state["context_override"]:
            bpy.ops.transform.translate(value=delta_to_apply.position, orient_type="GLOBAL")
            bpy.ops.transform.rotate(value=-angle, center_override=pivot_position, orient_matrix=create_z_orient(axis))
    ...
    is_gizmo_event = getattr(event, "handle_type", None) or settings["gizmo.transform_handles.type"]

    movable_bones, rotate_only_bones = [], []
    for bone in bones:
        is_disconnected_bone = bone.parent is None or not bone.bone.use_connect
        if is_gizmo_event or (is_disconnected_bone and not T["lock_bone_position"]):
...
def transform_bones_without_constraints(bones, transform_m, transform_m_inv, world_rot, event, delta_to_apply):
    for bone in bones:
        ...
        if getattr(event, "handle_type", None) is not None or (  # i.e. is from transform handles/gizmo
            (bone.parent is None or not bone.bone.use_connect) and not T["lock_bone_position"]
        ):
...
def transform_bone(
    ...
    if el_type == "BOTH":
        ...
        if T["lock_bone_position"]:
            delta_to_apply.position = Vector()
...
def on_cloned(self, event_name, cloned_objects):
    "This works by listening to clone events (during a grab lifecycle), and skipping undo entry creation if cloning. That's clone.py's responsibility."

    T["has_cloned"] = True

    # Update transform state if we're currently grabbing
    if T["object_to_transform"] and len(cloned_objects) > 0:
        T["transform_elements"] = T["preselected_targets"] = set(cloned_objects)
        T["object_to_transform"] = cloned_objects[0]


def on_grab_start(self, event_name, event):
    T["has_cloned"] = False

    configure_transform_targets(self, event)

    root.add_event_listener("fb.clone", on_cloned)


def on_grab_end(self, event_name, event):
    T["has_cloned"] = False
...
def get_selected_elements(elements=None, active_ob=None):
    ...
    elif ob.mode == "EDIT" and ob.type == "MESH":
        bm = get_bmesh()
        old_elements = elements
        elements = []
        for i, el_type in old_elements:
            if el_type == BMVert:
    ...
    if ob.type == "ARMATURE" and ob.mode in ("EDIT", "POSE"):
        ...
        if bpy.app.version >= (5, 0, 0):
            for bone_name, el_type in elements:
                if ob.mode == "POSE":
                    main_bone = ob.pose.bones[bone_name]
                    if el_type in ("", "BOTH") and main_bone.select:
                        selected_bones.add((bone_name, "BOTH"))
                else:
                    main_bone = ob.data.edit_bones[bone_name]
                    if el_type in ("", "BOTH") and (
                        main_bone.select or (main_bone.select_head and main_bone.select_tail)
                    ):
                        selected_bones.add((bone_name, "BOTH"))
    ...
    selected_elements = {e for e in elements if e.select}
    if ob.mode == "EDIT" and ob.type == "MESH":
        selected_elements = {(e.index, type(e)) for e in selected_elements}
...
def dispatch_event(self, event_name, event):
    ...
    elif self.mode == "EDIT" and self.type == "MESH":
        bm = get_bmesh()
        event.sub_targets = {bm.verts[i] for i, _ in transform_state["transform_elements"]}
    else:
...
def pivoted_rotate(pivot_pos, pivot_rot, cursor_pos, delta_to_apply):
    # work in relative-space
    prev_cursor_pos = cursor_pos - delta_to_apply.position - pivot_pos
    curr_cursor_pos = cursor_pos - pivot_pos

    prev_cursor_pos_after_rot = delta_to_apply.rotation @ prev_cursor_pos

    if prev_cursor_pos_after_rot.length_squared == 0 or curr_cursor_pos.length_squared == 0:
        effective_rot = Quaternion()
    else:
        effective_rot = prev_cursor_pos_after_rot.rotation_difference(curr_cursor_pos)

    new_rot = effective_rot @ delta_to_apply.rotation @ pivot_rot

    return quat_diff(new_rot, pivot_rot)


def constrain_for_gizmo(self, event):
    if T["transform_gizmo"] is None:
        # gizmos are shown only for 'select' tool, but objects can be grabbed during other tools as well (e.g. measure)
        return

    if settings["gizmo.transform_handles.type"] == "translate":
        event.pose_delta.rotation = Quaternion()
        event.pose_delta.scale_factor = 1
    elif settings["gizmo.transform_handles.type"] == "rotate":
        if self.type == "ARMATURE" and self.mode in ("EDIT", "POSE"):
            event.pose_delta.scale_factor = 1
            T["lock_bone_position"] = True
        else:
            base_pos = T["transform_gizmo"].position_world
            base_rot = T["transform_gizmo"].rotation_world

            event.pose_delta = Pose(rotation=pivoted_rotate(base_pos, base_rot, event.pivot_position, event.pose_delta))
            event.pivot_position = Vector(base_pos)
    elif settings["gizmo.transform_handles.type"] == "scale":
        event.pivot_position = Vector(T["transform_gizmo"].position_world)
        event.pose_delta.position = Vector()
        event.pose_delta.rotation = Quaternion()
