        ...
        if ob.type == "ARMATURE":
            if bpy.app.version >= (5, 0, 0):
                if ob.mode == "POSE":
                    ...
                    return np.array([[b.select, b.select, b.select] for b in bones], dtype=bool)
...
def finish_selection():
    ...
    if state_before_selection is None or curr_state is None:
        select_made_changes = False
    elif len(curr_state) == len(state_before_selection):
