...
from bl_xr import Node, Button, Image, Text
from bl_xr.consts import WHITE
...
Node.STYLESHEET.update(
    {
        ...
        ".controller_bg": {
            "scale": 0.09,
            "position": Vector((-0.045, -0.09, 0)),
            "opacity": 0.85,
            "background": (0.12, 0.46, 0.90, 1),
            "border": (0.003, (0.95, 0.96, 0.97, 1)),
            "border_radius": 0.012,
        },
    }
)
...
controller_alt = Node(
    ...
    child_nodes=[
        ...
        Node(
            child_nodes=[
                Text(
                    id="scene_scale_label",
                    text="Scale: 1:10",
                    style={"color": WHITE},
                    intersects=None,
                ),
            ],
            position=Vector((-0.015, -0.06, 0.001)),
            scale=0.018,
        ),
    ],
)
...
def on_alt_controller_update(self):
    global scene_scale_label

    if scene_scale_label is None:
        scene_scale_label = root.q("#scene_scale_label")

    scale = xr_session.viewer_scale
    scene_scale_label.text = f"Scale: 1:{scale:.0f}" if scale >= 1 else f"Scale: {1/scale:.0f}:1"
        ...
        for area in screen.areas:
            if area.type == "VIEW_3D":
                with bpy.context.temp_override(window=window, area=area):
                    bpy.ops.blendlinkxr.xr_toggle("INVOKE_DEFAULT")
                break
...
controller_alt.add_event_listener("pointer_main_press_start", lambda *x: None)  # hack to get laser pointer visibility
...
def on_undo_redo_press(self, event_name, event):
    from blendlinkxr.undo_redo import JOYSTICK_THRESHOLD
