...
def _do_check_update(ref, install_update):
    ...
    try:
        ...
        bl_dir = path.join(path.expanduser("~"), ".blendlinkxr")
        os.makedirs(bl_dir, exist_ok=True)

        version_file = path.join(path.dirname(__file__), "..", "version.json")
        license_file = path.join(bl_dir, ".license")
        ...
        version_different = latest_available_version[:2] > curr_version[:2]
        hash_different = latest_available_commit != curr_commit

        if version_different:
            log.warning("BlendLink XR needs to be updated!")
            needs_update = True

        update_checking_state = None

        if (version_different or hash_different) and (settings["app.update.auto_update"] or install_update):
            log.warning("Auto-updating BlendLink XR..")
            download_and_install()
    except Exception as e:
        log.exception(f"Error while checking for an update to BlendLink: {e}")
    finally:
        update_checking_state = None
...
def download_and_install():
    ...
    tmp_filepath = path.join(tempfile.gettempdir(), "blendlinkxr.zip")
    unzip_dirpath = path.join(tempfile.gettempdir(), "blendlinkxr_update")
...
def fail_with_message(action, msg):
    ...
    update_error = f"Oops! Could not {action} the update for BlendLink. Error message: {msg}"
    update_installing_state = "ERROR"


class ApplyUpdateOperator(bpy.types.Operator):
    bl_idname = "blendlinkxr.apply_update"
    bl_label = "BlendLink Apply Update"
...
class CheckForUpdateOperator(bpy.types.Operator):
    bl_idname = "blendlinkxr.check_update"
    bl_label = "BlendLink Check Update"
