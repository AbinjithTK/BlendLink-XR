...
def on_transform_start(self, event_name, event):
    ...
    from blendlinkxr.gizmos import active_gizmos, enable_gizmo

    if len(event.targets) != 1 or getattr(event.targets[0], "type", None) != "CAMERA":
        disable_gizmo_upon_release = False
        return
    ...
    cam_preview.camera_id = cam.name if cam else None
...
def release_camera():
    ...
    from blendlinkxr.gizmos import disable_gizmo

    cam_preview = root.q("#camera_preview")
    if cam_preview:
        cam_preview.camera_id = bpy.context.scene.camera.name if bpy.context.scene.camera else None
