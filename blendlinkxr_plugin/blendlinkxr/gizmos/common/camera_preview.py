...
from blendlinkxr.utils import desktop_viewport, log
...
class CameraPreview(Node):
    ...
    def draw_camera(self):
        ...
        if self.camera_id is not None and self.camera_id not in bpy.data.objects:
            log.info(f"camera preview: camera not found: {self.camera_id}. Looking for a new one.")
            self.camera_id = None  # invalid or deleted camera, reset to default
        ...
        if self.camera_id is None:
            if bpy.context.scene.camera is not None:
                self.camera_id = bpy.context.scene.camera.name
            else:
                self.camera_id = next(
                    (ob.name for ob in bpy.context.scene.objects if getattr(ob, "type", None) == "CAMERA"), None
                )
                if self.camera_id is None:
                    return
        ...
        context = bpy.context
        scene = context.scene
        cam = bpy.data.objects[self.camera_id]
        ...
        projection_matrix = cam.calc_matrix_camera(context.evaluated_depsgraph_get(), x=W, y=H)
