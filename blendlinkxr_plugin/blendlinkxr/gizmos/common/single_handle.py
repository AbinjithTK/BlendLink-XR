import bl_xr
from bl_xr import xr_session, Node, Plane, Cube, Sphere
...
class SingleHandle(Node):
    "A knob that can be dragged in any direction (and dispatches handle drag events to itself)"

    def __init__(self, knob_type="cube", **kwargs):
        ...
        size = settings["gizmo.single_handle.size"]

        if knob_type == "cube":
            self.knob = Cube(size=size)
        elif knob_type == "sphere":
            self.knob = Sphere(radius=size)
        elif knob_type == "plane":
            self.knob = Plane(width=size, height=size)
