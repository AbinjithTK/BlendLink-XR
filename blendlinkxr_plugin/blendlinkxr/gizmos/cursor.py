...
class Cursor(Node):
    CURSOR_DEFAULTS_KEYS = {
        ...
        "draw.grease_pencil": ("gp.default_cursor_size", "select.default_cursor_color"),
        "draw.gp_erase": ("erase.default_cursor_size", "erase.default_cursor_color"),
        "draw.gp_smooth": ("gp.default_cursor_size", "select.default_cursor_color"),
        "draw.gp_line": ("gp.default_cursor_size", "select.default_cursor_color"),
        ...
        "draw.text": ("shape.default_cursor_size", "select.default_cursor_color"),
        ...
        "measure": ("shape.default_cursor_size", "select.default_cursor_color"),
    }
    ...
    def on_cursor_resize(self, event_name, event):
        ...
        if settings["world_nav.nav_mode"] in ("WALK", "FLY"):
            return
...
def enable_gizmo():
    ...
    root.add_event_listener("joystick_x_main_press", cursor.on_cursor_resize)
    root.add_event_listener("joystick_x_main_end", cursor.on_cursor_resize_end)


def disable_gizmo():
    root.remove_child(cursor)

    root.remove_event_listener("joystick_x_main_press", cursor.on_cursor_resize)
    root.remove_event_listener("joystick_x_main_end", cursor.on_cursor_resize_end)
