...
def on_navigate(self, event_name, event):
    ...
    bpy.context.scene.blendlinkxr_viewer_position = xr_session.viewer_location.to_tuple()
    bpy.context.scene.blendlinkxr_viewer_rotation = (r.w, r.x, r.y, r.z)
    bpy.context.scene.blendlinkxr_viewer_scale = xr_session.viewer_scale


def on_xr_start(self, event_name, operator):
    has_saved_view = not is_equal((0.0, 0.0, 0.0, 0.0), bpy.context.scene.blendlinkxr_viewer_rotation)
    if has_saved_view:
        xr_session.viewer_location = Vector(bpy.context.scene.blendlinkxr_viewer_position)
        xr_session.viewer_rotation = Quaternion(bpy.context.scene.blendlinkxr_viewer_rotation)
        xr_session.viewer_scale = bpy.context.scene.blendlinkxr_viewer_scale
