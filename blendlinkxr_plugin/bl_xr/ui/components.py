...
class Mesh:
    ...
    def new_plane(width: float = 1, height: float = 1) -> Mesh:
        verts, faces = make_plane(width, height)
        return Mesh(verts, faces)
...
class Plane(Node):
    ...
    @width.setter
    def width(self, width: float = 1):
        self._width = width
        self.mesh = Mesh.new_plane(width, self._height)

    @height.setter
    def height(self, height: float = 1):
        self._height = height
        self.mesh = Mesh.new_plane(self._width, height)

    @property
    def bounds_local(self) -> Bounds:
        half_size = 0.5 * Vector((self.width, self.height, 0.002))
        return Bounds(-half_size, half_size)
...
class Image(Node):
    ...
    def src(self, img_src):
        ...
        if isinstance(self._src, str):
            img_path = Image.get_abs_path(self._src)
        ...
        elif callable(self._src):
            self._shader = self._src()

    @property
    def bounds_local(self) -> Bounds:
        return Bounds(VEC_ZERO, Vector((self.width, self.height, 0)))

    def get_abs_path(img_src):
        img_path = path.join(Image.base_dir, img_src) if Image.base_dir else img_src
        return path.abspath(img_path)
...
class Text(Node):
    ...
    def draw(self):
        ...
        for i, line in enumerate(lines):
            blf.position(font_id, 0, -i * line_height, 0)
...
class Grid2D(Node):
    def __init__(
        ...
        **kwargs,
        ...
        self.reverse_y_order = reverse_y_order
    ...
    def _update_layout(self):
        ...
        num_rows = len(self.child_nodes) // num_cols + (1 if len(self.child_nodes) % num_cols > 0 else 0)

        for i, node in enumerate(self.child_nodes):
            row_idx, col_idx = i // num_cols, i % num_cols
            if self.reverse_y_order:
    ...
    def insert_before(self, new_node, reference_node):
        super().insert_before(new_node, reference_node)
...
class Button(Node):
    def __init__(self, icon: str, tooltip: str = "", **kwargs):
        ...
        self.enabled_checker = kwargs.get("enabled_checker", None)
    ...
    def highlight(self, state: bool):
        ...
        if state and not self.disabled:
            self.icon.style["background"] = (0.12, 0.46, 0.90, 1.0)
            self.icon.style["border"] = (0.015 * self.icon.width, (0.08, 0.35, 0.80, 1.0))
    ...
    def update(self):
        if self.highlight_checker and callable(self.highlight_checker) and not self._stop_highlight_checker:
            should_highlight = self.highlight_checker()
            self.highlight(should_highlight)

        if self.enabled_checker and callable(self.enabled_checker):
            should_enable = self.enabled_checker()
            self.disabled = not should_enable
            self.icon.style["opacity"] = 1.0 if should_enable else 0.1
