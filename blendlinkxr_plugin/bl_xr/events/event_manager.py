...
def on_xr_action(bl_event: Bl_Event):
    ...
    # Suppress trigger/squeeze when MX Ink is docked (pen in charger)
    try:
        from blendlinkxr.settings_manager import settings
        if settings.get("mx_ink.docked", False) and base_event.button_name in ("trigger", "squeeze"):
            return
    except ImportError:
        pass
    ...
    for e in events:
        if e.type == "trigger_main_start":
            prevent_trigger_events_on_raycast = any(
                node.prevent_trigger_and_pointer_press_conflict for node in intersections.curr["raycast"].keys()
            )
