...
from bl_xr.consts import VEC_FORWARD, VEC_ONE
        ...
        if event_both.button_name in _events_tracking:
            if "_end" in event_main.type and "_end" in event_alt.type and "_end" in event_both.type:
                ...
                if log.isEnabledFor(logging.DEBUG):
                    log.debug(
                        f"Transitioning from two-handed to single. Transferring intersections: {event_pose_data.start_intersection_targets}, {event_pose_data.start_intersection_sub_targets}"
                    )

                _events_tracking[event.button_name] = HighLevelEvent(
                    curr_pointer_pose,
                    curr_pointer_pose,
                    event_pose_data.is_dragging,
                    event_pose_data.start_intersection_targets,
                    event_pose_data.start_intersection_sub_targets,
                )
            ...
            if event_main.button_name in _events_tracking:
                existing_event = event_main
            elif event_alt.button_name in _events_tracking:
                existing_event = event_alt

            targets = None
            if existing_event:  # transitioning from single-handed to two-handed
                targets = _events_tracking[existing_event.button_name].start_intersection_targets
                sub_targets = _events_tracking[existing_event.button_name].start_intersection_sub_targets
                is_dragging = _events_tracking[existing_event.button_name].is_dragging

                del _events_tracking[existing_event.button_name]
            else:
                is_dragging = False

            if targets is None:
                targets = list(intersections.curr["bounds"].keys()) if intersections.curr["bounds"] else None
                sub_targets = intersections.sub_targets

            if existing_event and log.isEnabledFor(logging.DEBUG):
                log.debug(
                    f"Transitioning from single to two-handed. Transferring intersections: {targets}, {sub_targets}"
                )
...
def is_within_threshold(pose_delta: Pose) -> bool:
    ...
    scale_factor = pose_delta.scale_factor
    if not isinstance(scale_factor, Vector):
        scale_factor *= VEC_ONE
    ...
    return (
        ...
        and abs(scale_factor.x - 1) <= SCALE_SNAP_THRESHOLD
        and abs(scale_factor.y - 1) <= SCALE_SNAP_THRESHOLD
        and abs(scale_factor.z - 1) <= SCALE_SNAP_THRESHOLD
    )
