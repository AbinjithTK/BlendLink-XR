...
def dispatch_events(events: Event | list[Event]):
    ...
    for event in events:
        ...
        for target in targets:
            ...
            # skip pointer press events for disabled Node objects
            if (
                isinstance(target, Node)
                and target.disabled
                and event.type in ("pointer_main_press_start", "pointer_main_press_press", "pointer_main_press_end")
            ):
                continue
            ...
            if target != root and isinstance(target, Node):
                ...
                try:
                    ...
                    log.error(f"Error dispatching to: {target}")
