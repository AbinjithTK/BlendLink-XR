...
import bl_xr
from bl_xr import xr_session, ControllerEvent, Node
from bl_xr.events.make_events import controller
from bl_xr.utils import raycast, intersects, is_within_fov, log, get_node_breadcrumb, has_pointer_event_listeners
...
def update_bounds_intersection_cache(base_event: ControllerEvent, object_bounds=False, ui_bounds=False):
    ...
    if log.isEnabledFor(logging.DEBUG):
        log.debug(f'Bounds intersections updated: {curr["bounds"]}. Sub targets: {sub_targets}')
        _print_ui_breadcrumbs(curr["bounds"])
...
def update_raycast_intersection_cache(base_event: ControllerEvent, object_raycast=False, ui_raycast=False):
    ...
    if node and (is_within_fov(point) or node in prev["raycast"]) and has_pointer_event_listeners(node):
        curr["raycast"][node] = point

    # if curr["raycast"]:
    #     print("intersecting", get_node_breadcrumb(node))
    #     bl_xr.root.q("#debug").position = point

    if log.isEnabledFor(logging.DEBUG):
        log.debug(f'Raycast intersections updated: {curr["raycast"]}')
        _print_ui_breadcrumbs(curr["raycast"])
...
def _print_ui_breadcrumbs(intersections):
    ui_nodes = [n for n in intersections if isinstance(n, Node)]
    if not ui_nodes:
        return

    log.debug("Breadcrumbs:")
    for node in ui_nodes:
        log.debug(f"- {get_node_breadcrumb(node)} (created: {node._creation_stack})")
