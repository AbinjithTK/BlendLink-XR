...
def make_class_event_aware(clazz):
    ...
    def clear_event_listeners():
        EventAware.clear_event_listeners(clazz)
    ...
    setattr(clazz, "clear_event_listeners", clear_event_listeners)
...
def has_pointer_event_listeners(node):
    if node is None or node == bl_xr.root:
        return False

    if hasattr(node, "event_listeners") and any(
        listener.startswith("pointer_") for listener in node.event_listeners.keys()
    ):
        return True

    return has_pointer_event_listeners(node.parent)
