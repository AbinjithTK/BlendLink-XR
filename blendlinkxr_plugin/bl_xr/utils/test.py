...
from bl_xr.utils import vec_equal, quat_equal, from_blender_axis_system, to_blender_axis_system, is_equal
...
def make_bl_event(
    ...
    * value - float, [float, float]
    ...
    if getattr(make_bl_event, "pitched_rotation", False):
        rotation = [to_blender_axis_system(r) for r in rotation]
...
def make_tuple(val: Union[Vector, Quaternion, tuple]):
...
def xr_action_from_bl_event(bl_event):
    "Inverse of make_bl_event. Converts a bl_event into arguments for make_bl_event()"

    xr = bl_event.xr

    def h(hand):
        return hand.replace("/user/hand/", "")

    if xr.bimanual:
        hand = [h(xr.user_path), h(xr.user_path_other)]
        value = [xr.state[0], xr.state_other[0]]
        position = [make_tuple(xr.controller_location), make_tuple(xr.controller_location_other)]

        rotation = [xr.controller_rotation, xr.controller_rotation_other]
        # rotation = [to_blender_axis_system(r) for r in rotation]
        rotation = [make_tuple(r) for r in rotation]
    else:
        hand = h(xr.user_path)
        value = xr.state[0]
        position = make_tuple(xr.controller_location)

        rotation = xr.controller_rotation
        # rotation = to_blender_axis_system(rotation)
        rotation = make_tuple(rotation)

    return [xr.action, bl_event.value, hand, value, position, rotation]


def deserialize_xr_action(action, press_type, hand, value, position, rotation):
    if isinstance(hand, list):
        position = [Vector(v) for v in position]
        rotation = [Quaternion(q) for q in rotation]
    else:
        position = Vector(position)
        rotation = Quaternion(rotation)

    return [action, press_type, hand, value, position, rotation]


def xr_controller_move_from_bl_event(bl_event):
    hand, position, rotation, _ = bl_event

    # rotation = to_blender_axis_system(rotation)

    return [hand, make_tuple(position), make_tuple(rotation)]


def deserialize_xr_controller_move(hand, position, rotation):
    position = Vector(position)
    rotation = Quaternion(rotation)

    return [hand, position, rotation]


def mouse_move_from_bl_event(bl_event):
    return [bl_event.mouse_x, bl_event.mouse_y]
...
def make_bezier_curve(position=Vector(), undo_entry=True, name=None):
    prev_active_obj = bpy.context.view_layer.objects.active

    bpy.ops.curve.primitive_bezier_circle_add(location=position)
    ...
    if undo_entry:
        bpy.ops.ed.undo_push(message="make bezier curve")

    if new_ob == prev_active_obj:
        raise Exception("Couldn't create bezier curve")
    return new_ob
...
def make_text(name: str = "Font", text: str = "Font", position: Vector = None, rotation: Quaternion = None):
    rotation = rotation if rotation is not None else Quaternion((1, 0, 0), radians(90))
    position, rotation = make_defaults_if_needed(position, rotation)
    ...
    bpy.ops.ed.undo_push(message="make font")
