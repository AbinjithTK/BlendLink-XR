...
def get_caller_stack(from_idx=-4, to_idx=-2):
    import traceback
    import os

    stack = traceback.extract_stack()
    if to_idx == -1:
        frames = stack[from_idx:]
    else:
        frames = stack[from_idx : to_idx + 1]
    return [f"{os.path.basename(frame.filename)}:{frame.lineno} in {frame.name}" for frame in frames]
