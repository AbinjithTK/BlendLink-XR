...
def intersects_object_mesh(ob, center, shape, size):
    ...
    if ob.type in ("CURVE", "FONT") or len(ob.data.polygons) > 0:
...
intersects_object_font = intersects_object_mesh
...
def check_custom_bone_shape_mesh(ob, center, shape, size, mat_world, bone_length):
    ...
    for e in bm.edges:
        ...
        if d.length <= size:
            intersecting_elements.add(e)
            return list(intersecting_elements)  # short-circuit to save time

    if len(intersecting_elements) > 0:
        return list(intersecting_elements)


def check_custom_bone_shape_curve(ob, center, shape, size, mat_world, bone_length):
    intersecting_elements = set()
    ...
    for spline in ob.data.splines:
        if spline.type == "BEZIER":
            ...
            for i in range(num_segments):
                i0 = i
                i1 = (i + 1) % n
                p0 = spline.bezier_points[i0]
                p1 = spline.bezier_points[i1]

                evaluated_pts = interpolate_bezier(p0.co, p0.handle_right, p1.handle_left, p1.co, spline.resolution_u)

                for i in range(len(evaluated_pts) - 1):
                    v0 = evaluated_pts[i]
                    v1 = evaluated_pts[i + 1]
                    v0_world = mat_world @ (v0 * bone_length)
                    v1_world = mat_world @ (v1 * bone_length)
                    nearest_pt = nearest_point_on_line_segment(center, v0_world, v1_world)
                    d = nearest_pt - center
                    if d.length <= size:
                        intersecting_elements.add(p0)
                        return list(intersecting_elements)  # short-circuit to save time
        else:
            pass
...
def get_intersecting_bones(ob, center, shape, cursor_size, armature, bones):
    ...
    if bpy.app.version >= (4, 0):
        has_bone_layers = len(ob.data.collections) > 0
        first_collection_name = ob.data.collections[0].name if has_bone_layers else None
        visible_layers = {c.name for c in ob.data.collections if c.is_visible}
    else:
        has_bone_layers = True
        visible_layers = np.array(ob.data.layers)

    intersecting_elements = []
    for bone in bones:
        bone_actual = bone.bone if isinstance(bone, PoseBone) else bone
        if bone_actual.hide:
            continue

        # check bone layer/collection visibility
        if has_bone_layers:
            if bpy.app.version >= (4, 0):
                bone_layers = bone_actual.collections.keys()
                if not bone_layers:
                    bone_layers = [first_collection_name]
                if not visible_layers.intersection(bone_layers):
                    continue
            else:
                bone_layers = np.array(bone_actual.layers)
                if not np.any(bone_layers & visible_layers):
                    continue
        ...
        if isinstance(bone, PoseBone) and bone.custom_shape:
            ...
            check_fn = None
            if bone.custom_shape.type == "MESH":
                check_fn = check_custom_bone_shape_mesh
            elif bone.custom_shape.type == "CURVE":
                check_fn = check_custom_bone_shape_curve

            if check_fn and check_fn(bone.custom_shape, center, shape, cursor_size, transform_matrix, bone_length):
...
def raycast_individual_node(node: Node, ray_start, ray_dir, ray_dist=math.inf) -> tuple[bool, Vector, Vector, float]:
    ...
    * currently only works for 2D bounds, not 3D bounds
    ...
    if (
        ...
        # log.debug(f"out of bounds: {get_node_breadcrumb(node)}, {bounds_local=}, {intersection_local=}")
        return NOT_INTERSECTED
    ...
    # log.debug(f"intersected: {get_node_breadcrumb(node)}")
