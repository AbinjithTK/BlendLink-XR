...
class Node(EventAware):
    ...
    def __init__(self, **kwargs):
        ...
        self._prevent_trigger_and_pointer_press_conflict: bool = kwargs.get(
            "prevent_trigger_and_pointer_press_conflict", None
        )
        ...
        if log.isEnabledFor(logging.DEBUG):
            from .utils.misc_utils import get_caller_stack

            self._creation_stack = get_caller_stack(from_idx=-4, to_idx=-3)[0]
    ...
    def insert_before(self, new_node: Node, reference_node: Node):
        if not isinstance(new_node, Node):
            raise ValueError(f"Expected a new_node of type Node. Got: {type(new_node)}")

        if reference_node not in self.child_nodes:
            raise ValueError("reference_node is not a child of this Node.")

        if new_node._parent:
            if new_node in new_node._parent.child_nodes:
                new_node._parent.remove_child(new_node)

        index = self.child_nodes._nodes.index(reference_node)
        self.child_nodes._nodes.insert(index, new_node)
        new_node._parent = self

    @property
    def first_child(self) -> Node:
        return self.child_nodes[0] if len(self.child_nodes) > 0 else None
    ...
    @property
    def prevent_trigger_and_pointer_press_conflict(self) -> bool:
        """
        Improves the usability of UI-based scenes. This flag:
        1. Prevents `trigger` events from firing if already pointing at (i.e. raycast intersecting) this Node (or its children).
        2. Prevents `pointer_press` events from firing if the trigger was already pressed before raycast intersection started with the Node (or its children).
        """

        if self._prevent_trigger_and_pointer_press_conflict is None:
            return True if self.parent is None else self.parent.prevent_trigger_and_pointer_press_conflict

        return self._prevent_trigger_and_pointer_press_conflict

    @prevent_trigger_and_pointer_press_conflict.setter
    def prevent_trigger_and_pointer_press_conflict(self, v: bool):
        self._prevent_trigger_and_pointer_press_conflict = v
